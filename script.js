...
async function handleAuth(event) {
    ...
    try {
        ...
        if (isLoginMode) {
            ...
            // البحث في البيانات المحلية أولاً (مع تحسين البحث)
            let user = users.find(u => {
                // البحث باستخدام البريد الإلكتروني فقط
                if (u.email === email) {
            ...
            try {
                ...
                if (response.status === 200) {
                    ...
                    if (existingUserIndex === -1) {
                        ...
                        // تحديث كلمة المرور والبيانات الأخرى
                        users[existingUserIndex] = {
                            ...users[existingUserIndex],
                            ...user,
                            password: password // حفظ كلمة المرور الحالية
                        };
                    }
                    ...
                    console.log('⚠️ قاعدة بيانات المستخدمين غير موجودة في السيرفر');
                    
                    // محاولة البحث في البيانات المحلية مرة أخرى مع رسالة مساعدة
                    ...
                    if (user) {
                        ...
                        alert('❌ البريد الإلكتروني أو كلمة المرور غير صحيحة\n\nℹ️ تلميحات:\n1. تأكد من صحة البريد الإلكتروني وكلمة المرور\n2. إذا نسيت كلمة المرور، جرب إنشاء حساب جديد\n3. تأكد من أن الحساب موجود في البيانات المحلية');
                    }
                ...
                // حل بديل: استخدام البيانات المحلية فقط
                console.log('🔍 البحث في جميع البيانات المحلية المخزنة...');
                
                // تحميل البيانات المحلية مباشرة للتأكد
                const storedUsers = JSON.parse(localStorage.getItem('webaidea_users')) || [];
                console.log('المستخدمين المخزنين:', storedUsers);
                
                user = storedUsers.find(u => {
                    console.log('مقارنة:', u.email, 'مع', email);
                ...
                if (user) {
                    ...
                    updateUI();
                    closeModal();
                    alert(`🎉 مرحباً بعودتك ${user.name}! (اتصال محلي)\n\n⚠️ تم استخدام البيانات المحلية بسبب مشكلة في الاتصال بالسيرفر.`);
                } else {
                    // عرض جميع حسابات البريد الإلكتروني المخزنة للمساعدة في التصحيح
                    const storedEmails = storedUsers.map(u => u.email).join(', ');
                    console.log('البريد الإلكتروني المدخل:', email);
                    console.log('الحسابات المخزنة:', storedEmails);
                    
                    alert(`❌ البريد الإلكتروني أو كلمة المرور غير صحيحة\n\n🔍 تلميحات للتصحيح:\n1. البريد المدخل: ${email}\n2. عدد الحسابات المخزنة: ${storedUsers.length}\n3. إذا كنت قد أنشأت حساباً للتو، جرب إنشاء حساب جديد\n4. إذا نسيت كلمة المرور، لا يمكن استعادتها حالياً`);
                }
            }
            ...
            try {
                ...
                if (confirm('⚠️ مشكلة في الاتصال بالسيرفر. هل تريد إنشاء حساب محلياً؟\n\n(البيانات ستكون محلية فقط حتى يتم الاتصال بالسيرفر)')) {
                    ...
                    const newUser = {
                        ...
                        name: name,
                        email: email,
                        password: password, // حفظ كلمة المرور بشكل صريح
                        type: 'user',
                        joinDate: new Date().toISOString().split('T')[0]
                    };
